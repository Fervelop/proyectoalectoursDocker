    ...
    <div className="bg-card text-card-foreground rounded-xl shadow-md border border-border overflow-hidden sticky top-24 transition-colors duration-200">
      ...
      <div className="px-5 pb-5">
        ...
        <div className="text-center mb-5">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">@{usuario?.username || "viajero"}</p>
          <div className="flex items-center justify-center gap-1 mt-2 bg-primary/10 px-2.5 py-0.5 rounded-full w-max mx-auto border border-primary/20">
            <Star className="w-3 h-3 fill-primary text-primary" />
            <span className="text-[11px] font-bold text-primary uppercase tracking-wider">Viajero VIP</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            { value: reservas.length, label: "Reservas", color: "text-foreground" },
            { value: totalViajes, label: "Viajes", color: "text-primary" },
            { value: reservas.filter(r => r.estado === "pendiente").length, label: "Pendientes", color: "text-amber-600 dark:text-amber-400" },
          ].map(s => (
            <div key={s.label} className="bg-muted/40 rounded-lg py-2 text-center border border-border/50">
          ...
          <div className="space-y-2 mb-4 py-3 border-t border-b border-border/60">
            ...
            {clienteData.ciudad && (
              <div className="flex items-center gap-2.5 text-muted-foreground text-xs">
                ...
                <span className="truncate">{clienteData.ciudad}, {clienteData.pais}</span>
              </div>
            )}
          </div>
        )}
        ...
        {proximaReserva && (
          <div className="bg-neutral-900 dark:bg-muted text-neutral-100 dark:text-foreground rounded-xl p-3.5 mb-4 border border-neutral-800 dark:border-border shadow-sm">
            <div className="flex items-center gap-1.5 mb-1.5 text-primary dark:text-primary">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest">Próximo viaje</span>
            </div>
            <p className="text-sm font-bold tracking-tight text-white dark:text-foreground">Paquete #{proximaReserva.id_paquete}</p>
          ...
          {tabs.map(tab => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 cursor-pointer ${isActive
              ...
              >
                ...
                <ChevronRight className={`w-4 h-4 transition-transform duration-200 ${isActive ? "rotate-90 text-primary-foreground" : "text-muted-foreground/50"}`} />
              </button>
            );
          })}
        </nav>
