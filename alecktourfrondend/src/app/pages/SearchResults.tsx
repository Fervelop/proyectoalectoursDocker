...
export default function SearchResults() {
  ...
  const [ciudadSearch, setCiudadSearch] = useState<string>("");
      ...
      .then((data) => {
        ...
        if (dest) {
          const exacta = data.find(
            (h) => h.ciudad.toLowerCase() === dest.toLowerCase()
          );
          if (exacta) {
            setCiudadFilter(exacta.ciudad);
          } else {
            const parcial = data.find((h) =>
              h.ciudad.toLowerCase().includes(dest.toLowerCase())
            );
            if (parcial) setCiudadFilter(parcial.ciudad);
          }
        }
      })
      .catch(console.error)
  ...
  const ciudadesFiltradas = ciudades.filter((c) =>
    c.toLowerCase().includes(ciudadSearch.toLowerCase())
  );
  ...
  const FilterContent = () => (
    <>
      {/* Calificación */}
      <div className="mb-6">
        <h3 className="font-semibold mb-3 text-sm uppercase tracking-wider text-muted-foreground/80">Calificación</h3>
      ...
      <div className="mb-6 border-t border-border/60 pt-5">
        <h3 className="font-semibold mb-3 text-sm uppercase tracking-wider text-muted-foreground/80">País</h3>
        <div className="space-y-2.5">
          <motion.label whileHover={{ x: 3 }} className="flex items-center gap-3 cursor-pointer group">
      ...
      {/* Ciudad con buscador */}
      <div className="mb-6 border-t border-border/60 pt-5">
        <h3 className="font-semibold mb-3 text-sm uppercase tracking-wider text-muted-foreground/80">Ciudad</h3>
              ...
              ciudadesFiltradas.map((ciudad) => (
                ...
                >
                  ...
                  <span className="text-sm text-foreground/90 group-hover:text-primary transition-colors">
                    {ciudadSearch
                      ? (() => {
                          const idx = ciudad.toLowerCase().indexOf(ciudadSearch.toLowerCase());
                          if (idx === -1) return ciudad;
                          return (
                            <>
                              {ciudad.slice(0, idx)}
                              <mark className="bg-primary/20 text-primary rounded px-0.5 font-semibold not-italic">
                                {ciudad.slice(idx, idx + ciudadSearch.length)}
                              </mark>
                              {ciudad.slice(idx + ciudadSearch.length)}
                            </>
                          );
                        })()
                      : ciudad}
        ...
        {ciudadFilter !== "all" && (
          <motion.div
            ...
            className="mt-3 flex items-center gap-2 px-3 py-1.5 bg-primary/10 border border-primary/20 rounded-lg w-fit"
          >
            <span className="text-xs font-semibold text-primary">{ciudadFilter}</span>
            <button
              onClick={() => { setCiudadFilter("all"); setCiudadSearch(""); }}
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            ...
            <div className="flex items-center justify-between gap-4">
              <p className="text-muted-foreground text-sm md:text-base font-medium">
                ...
                {ciudadFilter !== "all" && (
                  <span className="ml-2 text-primary font-semibold">en {ciudadFilter}</span>
                )}
                {startDate && endDate && (
                  <span className="ml-2 text-muted-foreground/70">· {startDate} → {endDate}</span>
                )}
                {people && (
                  <span className="ml-2 text-muted-foreground/70">· {people} {people === "1" ? "persona" : "personas"}</span>
                )}
              </p>
              <motion.button
              ...
              {loading ? (
                ...
                >
                  ...
                  <p className="text-muted-foreground text-sm mb-6 max-w-sm mx-auto">
                    {ciudadFilter !== "all"
                      ? `No encontramos hoteles en "${ciudadFilter}". Intenta con otra ciudad.`
                      : "Reajusta la calificación, ciudades o el país seleccionado."}
                  </p>
                  <motion.button
                    whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    onClick={limpiarFiltros}
