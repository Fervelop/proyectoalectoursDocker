  ...
  if (!hotel) return (
    <div className="min-h-screen bg-background transition-colors duration-200">
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 text-center">
        ...
        <h1 className="text-3xl font-bold text-foreground mb-4">Hotel no encontrado</h1>
        <Link to="/search" className="text-primary hover:underline font-medium">← Volver a explorar destinos</Link>
      </div>
    </div>
  );
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="flex gap-0.5">
                {Array.from({ length: 5 }, (_, i) => (
                  <Star key={i} className={`w-4 h-4 ${i < (hotel.calificacion ?? 0) ? "fill-chart-2 text-chart-2" : "fill-muted text-muted"}`} />
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 text-muted-foreground text-sm">
              ...
              <span className="hover:underline cursor-pointer">{hotel.direccion || 'Ubicación céntrica'}, {hotel.ciudad}</span>
              <span>•</span>
              <span className="text-primary font-medium cursor-pointer hover:underline">Ver mapa</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mb-10 h-[45vh] md:h-[55vh] rounded-2xl overflow-hidden group">
            ...
            {GALLERY_FILLERS.map((img, idx) => (
              <div key={idx} className="hidden md:block relative overflow-hidden h-full">
                <img
                  src={img}
                  alt="Instalaciones"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-10">
              ...
              {/* Acerca de */}
              <section>
                <h2 className="text-2xl font-semibold mb-4">Acerca de este alojamiento</h2>
                <div className="prose prose-sm md:prose-base dark:prose-invert text-muted-foreground">
                  <p>
                    Ubicado estratégicamente en <strong>{hotel.ciudad}</strong>, {hotel.nombre_hotel} ofrece una experiencia excepcional que combina comodidad moderna con la calidez del servicio local.
                    Ideal tanto para estancias de negocios como para escapadas de turismo.
                  </p>
                  <p className="mt-4">
                    Nuestros huéspedes destacan la limpieza, la atención del personal y nuestra ubicación privilegiada cerca de los principales puntos de interés. Siéntete como en casa mientras descubres lo mejor de {hotel.pais}.
                  </p>
                </div>
              </section>
              ...
              {caracteristicas.length > 0 && (
                <section>
                  <h2 className="text-2xl font-semibold mb-6">¿Qué ofrece este lugar?</h2>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-4">
                    {caracteristicas.map((hc) => {
                      ...
                      return (
                        <div key={hc.id_caracteristica} className="flex items-center gap-3">
                ...
                <section id="habitaciones">
                  <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-6 gap-4">
                    <div>
                      ...
                      <p className="text-muted-foreground mt-1 text-sm">Selecciona una habitación para proceder con la reserva.</p>
                    </div>
                  </div>
                    ...
                    {hotel.habitaciones.map((hab) => {
                      ...
                      return (
                        <motion.div
                          ...
                          whileHover={disponible && !seleccionada ? { y: -2 } : {}}
                          onClick={() => disponible && setSelectedHabitacion(hab)}
                          className={`relative flex flex-col md:flex-row border rounded-2xl overflow-hidden transition-all duration-300 bg-card ${!disponible
                        ...
                        >
                          ...
                          <div className="p-5 md:p-6 flex-1">
                            <div className="flex items-start justify-between mb-2">
                              ...
                              <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider ${ESTADO_STYLES[hab.estado]}`}>
                                {hab.estado}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 mb-4">
                              <span className="inline-flex items-center gap-1.5 bg-green-500/10 text-green-600 dark:text-green-400 px-2.5 py-1 rounded text-xs font-semibold">
                                <CheckCircle className="w-3.5 h-3.5" /> Cancelación Gratis
                              </span>
                              <span className="inline-flex items-center gap-1.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded text-xs font-semibold">
                                <UtensilsCrossed className="w-3.5 h-3.5" /> Desayuno Incluido
                              </span>
                            </div>

                            <div className="flex items-center gap-4 text-sm text-muted-foreground font-medium">
                              <span className="flex items-center gap-1.5"><Bed className="w-4 h-4 text-primary" /> Cama doble</span>
                              <span className="flex items-center gap-1.5"><Users className="w-4 h-4 text-primary" /> Máx. {hab.tipo_habitacion?.capacidad_personas} pers.</span>
                            </div>
                          </div>
                          ...
                          <div className="bg-muted/30 p-5 md:p-6 flex flex-col justify-center items-start md:items-end md:w-64 border-t md:border-t-0 md:border-l border-border">
                            <p className="text-xs text-muted-foreground font-medium mb-1">Precio por 1 noche</p>
                            <p className="text-2xl font-bold text-primary mb-1">
                              ${Number(hab.precio_noche).toLocaleString("es-CO")}
                            </p>
                            <p className="text-[11px] text-muted-foreground mb-4">Incluye impuestos y cargos</p>

                            <button
                              disabled={!disponible}
                              className={`w-full py-2.5 px-4 rounded-xl font-bold text-sm transition-all ${!disponible
                            ...
                            >
                              {!disponible ? "No disponible" : seleccionada ? "Seleccionada" : "Elegir"}
                            </button>
                          </div>
                        </motion.div>
                      );
              ...
              {/* Políticas simuladas */}
              <section>
                <h2 className="text-2xl font-semibold mb-6">Reglas del alojamiento</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex gap-4 p-5 rounded-2xl border border-border bg-card">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Check-in: 15:00 - 23:00<br />Check-out: Hasta las 12:00</p>
                    </div>
                  </div>
                  <div className="flex gap-4 p-5 rounded-2xl border border-border bg-card">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Se requiere presentar documento de identidad al momento del registro.</p>
                    </div>
                  </div>
                </div>
              </section>
            ...
            <div className="lg:col-span-1 relative">
              <div className="bg-card rounded-2xl border border-border p-6 shadow-xl sticky top-24 transition-colors">
                ...
                {/* Info superior */}
                <div className="mb-6">
                  <h3 className="text-xl font-bold leading-tight mb-2">{hotel.nombre_hotel}</h3>
                  <div className="text-sm text-muted-foreground space-y-1">
                    ...
                    <div className="flex justify-between">
                      <span>Contacto</span>
                      <span className="font-medium text-foreground">{hotel.telefono}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-muted/40 rounded-xl p-5 mb-6 border border-border">
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-semibold text-foreground">
                      {selectedHabitacion ? selectedHabitacion.tipo_habitacion?.nombre_tipo : "Selecciona habitación"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm text-muted-foreground mb-4">
                    <span>1 Noche, {selectedHabitacion ? selectedHabitacion.tipo_habitacion?.capacidad_personas : "2"} pers.</span>
                  </div>
                  ...
                  <div className="border-t border-border pt-4 flex justify-between items-end">
                    ...
                    <div className="text-right">
                      <span className="text-2xl font-black text-primary">
                        ${(selectedHabitacion?.precio_noche ?? precioMin ?? 0).toLocaleString("es-CO")}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <Link
                  ...
                  className={`flex justify-center items-center w-full py-4 text-base font-bold rounded-xl transition-all ${selectedHabitacion
                    ? "bg-primary text-primary-foreground shadow-md hover:scale-[1.02] hover:shadow-lg"
                    : "bg-primary/50 text-white cursor-pointer"
                    }`}
                >
                  {selectedHabitacion ? "Reservar ahora" : "Ver opciones de habitación"}
                </Link>
