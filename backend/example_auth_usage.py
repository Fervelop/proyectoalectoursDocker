...
def test_invalid_login():
    ...
    try:
        ...
        if response.status_code == 401:
            print_success(f"Sistema rechazó credenciales inválidas (401)")
...
def test_invalid_register():
    ...
    payload = {
        ...
        "correo_electronico": "esto-no-es-un-email",
    ...
    try:
        ...
        if response.status_code == 422:
            print_success(f"Sistema validó email inválido (422)")
            print_json(response.json(), "Response Body")
        else:
            print_error(f"Respuesta inesperada: {response.status_code}")
